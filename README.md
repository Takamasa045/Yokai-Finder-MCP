...
- **詳細図鑑 83体** … 日英バイリンガルの伝承・創作フック（出典付きの増補分あり）
...
| 図鑑 (`list_yokai` + `hasProfile` / `yokai_of_the_day`) | 83体の詳細プロフィール。`list_curated_yokai` は非推奨 |
...
| 変数 | 説明 |
|------|------|
| `YOKAI_FINDER_VERSION` | サーバーが報告するバージョン文字列（未設定時はビルド時の `0.5.0`） |
...
| `source` | 意味 |
|----------|------|
| `profile` | 図鑑83体のいずれか。伝承・豆知識など詳細あり |
...
引数なしだと、**JST の日付**から決まる「今日の1体」（詳細図鑑83体から）を紹介します。同じ日は同じ妖怪。プロフィール・伝承・創作フック・NDLのおすすめ書籍・ストーリープロンプトを一度に返します。
