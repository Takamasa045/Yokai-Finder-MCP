...
| 図鑑 (`list_curated_yokai` / `yokai_of_the_day`) | 50体の詳細プロフィール（要約・伝承・豆知識は日英併記） |
| 書籍 (`search_yokai_books`) | NDL OpenSearch で妖怪関連書を検索。ISBN があれば書影URL候補付き |
| 書影 (`get_cover_thumbnail`) | ISBN / 全国書誌番号から表紙画像URL候補を生成 |
| 基盤 | 公式 MCP Go SDK（stdio）、検索キャッシュ（TTL 5分・最大256件） |
...
| 変数 | 説明 |
|------|------|
| `YOKAI_FINDER_VERSION` | サーバーが報告するバージョン文字列（未設定時は `0.3.0`） |
...
| `source` | 意味 |
|----------|------|
| `profile` | 図鑑50体のいずれか。伝承・豆知識など詳細あり |
...
| `category` | 任意 | カテゴリ（例: `水系` `付喪神` `狐狸` `現代伝承`） |
| `region` | 任意 | 地域のヒント（例: `東北` `九州` `海`） |
...
### 4. `list_curated_yokai` — 妖怪図鑑（詳細50体）
...
| `limit` | 任意 | 最大件数（デフォルト 10、最大 50＝全図鑑） |
...
引数なしだと、日付から決まる「今日の1体」（詳細図鑑50体から）を紹介します。同じ日は同じ妖怪。プロフィール・伝承・創作フック・NDLのおすすめ書籍・ストーリープロンプトを一度に返します。
...
国立国会図書館 OpenSearch API で妖怪関連の書籍を検索します。索引・図鑑にない名前でも検索できます。ISBN が取れた書籍には `coverImageCandidates` が付きます。
...
`list_curated_yokai` / `yokai_of_the_day` の対象。データ: `internal/yokai/profiles_data.go`  
いずれも索引にも載り、`list_yokai` では `hasProfile: true` になります。

河童 / 天狗 / 雪女 / 九尾の狐 / 化け猫 / ぬりかべ / ろくろ首 / 海坊主 / 餓者髑髏 / アマビエ / 座敷童子 / 鵺 / 鎌鼬 / 一反木綿 / 天邪鬼 / のっぺらぼう / 垢嘗 / 提灯お化け / 唐傘お化け / 化け狸 / 猫又 / 木霊 / 山姥 / 鬼 / 酒呑童子 / ぬらりひょん / 豆腐小僧 / 子泣き爺 / 産女 / 濡女 / 絡新婦 / 土蜘蛛 / 輪入道 / 覚 / 泥田坊 / 目目連 / 青行燈 / 玉藻前 / 清姫 / 橋姫 / 船幽霊 / 見越入道 / 小豆洗い / 口裂け女 / 犬神 / 雷獣 / 一つ目小僧 / 茨木童子 / 人魚 / 付喪神

### 索引（161体）

`list_yokai` の対象。データ: `internal/yokai/index_data.go`  
上記50体に加え、古典・地方伝承・都市伝説・付喪神・瑞獣などを広く収録。
...
│   └── yokai/
│       ├── index_data.go    # 索引 161体
│       ├── profiles_data.go # 詳細図鑑 50体
│       ├── index.go         # 索引 API
│       └── profiles.go      # 図鑑 API
├── pkg/types/               # 共有型定義
├── go.mod
...
`internal/yokai/index_data.go` に `IndexEntry` を追記します。
...
`internal/yokai/profiles_data.go` に `Profile` を追記します。

- `Summary` / `FunFact` … 英語
- `SummaryJA` / `FunFactJA` … 日本語
- `SearchQuery` … NDL 向け日本語キーワード
- 索引に同名（`Name` または `NativeName`）があると `hasProfile: true` になります
- 50体を超える場合は `list_curated_yokai` の `limit` 最大値も合わせて見直してください
