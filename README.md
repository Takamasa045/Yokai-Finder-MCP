...
- **詳細図鑑 80体** … 日英バイリンガルの伝承・創作フック（出典付きの増補分あり）
...
| 図鑑 (`list_curated_yokai` / `yokai_of_the_day`) | 80体の詳細プロフィール（要約・伝承・豆知識は日英併記） |
...
| 変数 | 説明 |
|------|------|
| `YOKAI_FINDER_VERSION` | サーバーが報告するバージョン文字列（未設定時はビルド時の `0.4.0`） |
...
| `source` | 意味 |
|----------|------|
| `profile` | 図鑑80体のいずれか。伝承・豆知識など詳細あり |
...
### 4. `list_curated_yokai` — 妖怪図鑑（詳細80体）
...
引数なしだと、**JST の日付**から決まる「今日の1体」（詳細図鑑80体から）を紹介します。同じ日は同じ妖怪。プロフィール・伝承・創作フック・NDLのおすすめ書籍・ストーリープロンプトを一度に返します。
...
Resources: `yokai://yokai/{name}`（例: `yokai://yokai/河童`）で図鑑/索引カードを JSON として読めます。  
Prompts: `yokai_story` / `yokai_character_sheet`。
